...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    required_prob = 1
    for person in people:
        # collect all the info about the person
        person_prob = 1
        ...
        if not mother and not father:
            # with no parents, calculate the unconditional probability
            person_prob *= PROBS["gene"][num_genes]
        else:
            # if has parents in the data, calculate the conditional probability
            # getting it from the mother
        ...
        # consider the trait probability
        person_prob *= PROBS["trait"][num_genes][has_trait]
        required_prob *= person_prob
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    """
    # loop through people in probabilities
    for person in probabilities:
        # find the number of genes they have
        num_genes = 1 if person in one_gene else 2 if person in two_genes else 0
        # find if they have the traits
        has_trait = person in have_trait
        # update the probabilities with the given joint probability p
...
def normalize(probabilities):
    ...
    for person in probabilities:
        for keys in probabilities[person]:
            total_prob = sum(probabilities[person][keys].values())
            for key in probabilities[person][keys]:
                probabilities[person][keys][key] /= total_prob
