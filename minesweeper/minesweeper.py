...
class Sentence():
    ...
    def known_mines(self):
        ...
        """
        # if count is equal to the number of cells in a sentence, then all cells are mines
    ...
    def known_safes(self):
        ...
        """
        # If count is 0, then all of the cells are safe
        if self.count == 0:
            return self.cells
        return set()
    ...
    def mark_safe(self, cell):
        ...
        """
        # if a cell is safe, it is removed from the sentence but count remains the same
        if cell in self.cells:
            self.cells.remove(cell)
...
class MinesweeperAI():
    ...
    def neighbors(self, cell, count):
        """"Returns all the neighbors and count of mines around a cell"""
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if (i, j) in self.mines:
                    count = count - 1
                    continue
                # Otherwise add the cell to the set
                if 0 <= i < self.height and 0 <= j < self.width:
                    new_sentence_cells.add((i, j))
        return new_sentence_cells, count
...
if __name__ == "__main__":
    sentence = Sentence({(1, 2), (1, 3), (1, 4)}, 2)
    print(sentence)